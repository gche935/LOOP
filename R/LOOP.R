...
Invariance <- function(parEst, pest2, pest3, no.path, no.waves, lag, Type1, Type1Adj, X, Y, Z, W) {
  ...
  if (any(parEst[,4] == "MX2")) {
    for (j in 2:no.waves) {
      for (i in j:no.waves) {
        ...
        names(pest2A) <- paste("MX", i, "-MX", i-j+1, sep="")
        pest2 <- append(pest2, pest2A)
  ...
  if (any(parEst[,4] == "MX2")) {
    cat(rep("\n",5), "## ===== Identification of invariant latent means ===== ##")
...
LandD_Path <- function(parEst, pest2, no.path, MIset, no.compare, no.waves, p, X, Y, Z, W, a="X", b="Y", LDlag=1) {
  ...
  SumEst <- 0
  cat(rep("\n", 2), paste("# -- Path ", a, b, " coefficients -- #", sep=""))
  aa <- get(a)
  bb <- get(b)
...
LandD_PC <- function(parEst, pest2, no.path, MIset, no.compare, no.waves, p, X, Y, Z, W, a="X", b="Y", LDlag=1) {
  ...
  SumEst <- 0
  cat(rep("\n", 2), paste("# -- Proportional change ", a, b, " coefficients -- #", sep=""))
  aa <- get(a)
  bb <- get(b)
  for (i in 1: (no.path-LDlag+1)) {
...
LandD_eXX <- function(parEst, pest2, no.path, MIset, no.compare, no.waves, p, X, Y, Z, W, a="X") {
  ...
  SumEst <- 0
  cat(rep("\n", 2), paste("# -- Residual variance of e", a, a, " coefficients -- #", sep=""))
  aa <- get(a)
  for (i in 1: no.path) {
...
LandD_eXY <- function(parEst, pest2, no.path, MIset, no.compare, no.waves, p, X, Y, Z, W, a="X", b="Y") {
  ...
  SumEst <- 0
  cat(rep("\n", 2), paste("# -- Residual covariance of e", a, b, " coefficients -- #", sep=""))
  aa <- get(a)
  bb <- get(b)
  for (i in 1: no.path) {
    Clhs <- paste("w", aa, i+1, sep="")
...
LandD_iXX <- function(parEst, pest2, no.path, MIset, no.compare, no.waves, p, X, Y, Z, W, a="X") {
  ...
  SumEst <- 0
  cat(rep("\n", 2), paste("# -- Impulse variance of i", a, a, " coefficients -- #", sep=""))
  aa <- get(a)
  for (i in 1: no.waves) {
...
LandD_iXY <- function(parEst, pest2, no.path, MIset, no.compare, no.waves, p, X, Y, Z, W, a="X", b="Y") {
  ...
  SumEst <- 0
  cat(rep("\n", 2), paste("# -- Co-Movements of i", a, b, " coefficients -- #", sep=""))
  aa <- get(a)
  bb <- get(b)
  for (i in 1: no.waves) {
    Clhs <- paste("d", aa, i, sep="")
...
if (any(parEst[,4] == "eIXY2")) {
  SumEst <- 0
  cat(rep("\n", 2), paste("# -- Indicator residual covariance of cov", a, b, " coefficients -- #", sep=""))
  aa <- get(a)
  bb <- get(b)
  for (i in 1: no.path) {
...
LandD_MEAN <- function(parEst, pest2, no.path, MIset, no.compare, no.waves, p, X, Y, Z, W, a="X") {
  ...
  SumEst <- 0
  cat(rep("\n", 2), paste("# -- Latent means of w", a, " -- #", sep=""))
  aa <- get(a)
  for (i in 1: no.waves) {
    Clhs <- paste0(aa, i, sep="")
    TparEst <- parEst[parEst["label"] == paste0("M", a, i),]
    p.TparEst <- paste0("  M", a, i, ":  Latent mean of ", Clhs, " = ", format(round(TparEst["est"], digits=4), nsmall=4, scientific=FALSE),
                       ", p-value = ", format(round(TparEst["pvalue"], digits=4), nsmall=4, scientific=FALSE), sep="")
    cat("\n", p.TparEst)
    SumEst <- SumEst + TparEst["est"]
  } # end (for i)
  MeanEst <- SumEst/(no.waves)
  p.MeanEst <- paste("  Average intercepts across waves from T1 = ", format(round(MeanEst, digits=4), nsmall=4, scientific=FALSE), sep="")
  cat("\n", p.MeanEst)
  ...
  cat(rep("\n",2), paste("# -- Sets of invariant latent means ", aa, " -- #", sep=""))
...
CLPM <- function(measurement.model="NULL", rcs=FALSE, data.source, no.waves, lag=1, Type1=0.05, Type1Adj="BON", X, Y, Z="NULL", W = "NULL") {
  ...
  ## -- Rename Variables with suffix _T -- ##
  X <- paste0(X, "_T")
  Y <- paste0(Y, "_T")
  if (Z != "NULL") Z <- paste0(Z, "_T")
  if (W != "NULL") W <- paste0(W, "_T")
  ...
  sink('CLPM.txt')
    ...
    if (measurement.model == "NULL") {
      ...
      cat(rep("\n",2), "  # -- Estimate grand means of indicators -- #")
      for (i in 1:no.waves) {
...
#      PMI <- measurement.model
      ...
      if (rcs == FALSE) {
        ...
        for (i in 1:no.waves) {
          cat("\n", paste0("  w", X, i, " ~ 0*1", sep=""))
          cat("\n", paste0("  w", Y, i, " ~ 0*1", sep=""))
          if (Z != "NULL") {
            cat("\n", paste0("  w", Z, i, " ~ 0*1", sep=""))
          } # end (if Z)
          if (W != "NULL") {
        ...
        cat(rep("\n",2), "  # -- Estimate grand means of indicators -- #")
        for (i in 1:no.waves) {
          cat("\n", paste0("  ", X, i, " ~ MX", i, "*1", sep=""))
          cat("\n", paste0("  ", Y, i, " ~ MY", i, "*1", sep=""))
          if (Z != "NULL") {
            PMI <- gsub(Z, paste0("w", Z), PMI)
            cat("\n", paste0("  ", Z, i, " ~ MZ", i, "*1", sep=""))
          } # end (if Z)
          if (W != "NULL") {
            PMI <- gsub(W, paste0("w", W), PMI)
  ...
  if (measurement.model == "NULL") {
    ...
    if (rcs == FALSE) {
      ...
      suppressWarnings(corrected <- relcorr_single_item(syntax = CLPM, data = Data))
      syntax2 <- corrected$syntax
      data_comp   <- corrected$data
...
RICLPM <- function(measurement.model="NULL", rcs=FALSE, data.source, no.waves, lag=1, Type1=0.05, Type1Adj="BON", X, Y, Z="NULL", W = "NULL") {
  ...
  sink('RICLPM.txt')
    ...
    if (rcs == FALSE) {
      ...
      BX <- paste0("  RI", X, " =~ 1* ", X, "1", sep="")
      BY <- paste0("  RI", Y, " =~ 1* ", Y, "1", sep="")
      for (i in 2:no.waves) {
        BX <- paste0(BX, " +1* ", X, i, sep="")
        BY <- paste0(BY, " +1* ", Y, i, sep="")
      } # end (for i)
      cat("\n", BX)
      cat("\n", BY)
      if (Z != "NULL") {
...
#      PMI <- gsub(Y, paste0("w", Y), PMI)
  ...
  if (measurement.model == "NULL") {
    ...
    if (rcs == FALSE) {
      ...
      suppressWarnings(corrected <- relcorr_single_item(syntax = RICLPM, data = Data))
      syntax2 <- corrected$syntax
      data_comp   <- corrected$data
...
#'
#' Convert wide data format to long data format with the capability to create lagged (t-1) variables. Variables should be coded with "_T1, _T2" to indicate the timelines. A new "id" variable will be created as the grouping variable, and a new "time" variable will be created as the timing variable.
#'
...
wide2long <- function(data.source, no.waves, variables = c("X", "Y"), lag1=FALSE) {
  ...
  variables2 <- paste0(variables, "_T")

  if (lag1 == FALSE) {
    df_long <- suppressWarnings(reshape(data.source,
      direction = "long",
      idvar = "id",
      varying = lapply(variables2, function(x) paste0(x, 1:no.waves)),
      v.names = sub("_T$", "", variables2),
      sep = "_T",
      timevar = "time",
...
#'
#' Create lagged (t-1) variables in dataframe in long format with labels "variable_lag".
#'
...
attrition.bias.test <- function(data.source, variables = c("X", "Y"), no.waves=3) {
  ...
  for (j in 1:no.waves) {
    # Columns to check
    cols_to_check <- paste0(variables, "_T", j)
    # Check for all NA in specific columns using apply
    data.source[, paste0("all_na_T",j)] <- apply(data.source[, cols_to_check], 1, function(x) all(is.na(x)))
    data.source[, paste0("all_na_T",j)] <- as.numeric(data.source[, paste0("all_na_T",j)]) # 1=missing
    data.source[, paste0("all_na_T",j)] <- (data.source[, paste0("all_na_T",j)] -1)^2 # 0=missing
  } # end (for j)
  ...
  for (j in 1:no.waves) {
    no.response[j,1] <- j
    no.response[j,2] <- sum(data.source[,paste0("all_na_T",j)])
  ...
  for (i in 1:(no.waves-1)) {
    Gp1 <- paste0("data.source$group.response[data.source$all_na_T",i," == 1 & data.source$all_na_T",i+1," == 1] <- 0")
    Gp2 <- paste0("data.source$group.response[data.source$all_na_T",i," == 1 & data.source$all_na_T",i+1," == 0] <- 1")
    eval(parse(text = Gp1))
    eval(parse(text = Gp2))
    ...
    scores <- data.source[,paste0(variables, "_T", i)]
  ...
  for (i in 1:(no.waves-2)) {
    Gp1 <- paste0("data.source$group.response[data.source$all_na_T",i," == 1 & data.source$all_na_T",i+2," == 1] <- 0")
    Gp2 <- paste0("data.source$group.response[data.source$all_na_T",i," == 1 & data.source$all_na_T",i+2," == 0] <- 1")
    eval(parse(text = Gp1))
    eval(parse(text = Gp2))
