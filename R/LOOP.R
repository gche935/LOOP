...
# ==================== Creating Function "LCMSR" ==================== #
#' Function Latent Curve Model with Structural Residuals (LCMSR)
#'
#' Latent Curve Model with Structural Residuals (LCM-SR)
#'
#' @param data.source name of data.frame
#' @param no.waves number of waves (minimum = 3, must be grater than lag)
#' @param lag number of waves between two lags (minimum = 1, maximum = 4)
...
#' @param W name of variable W (Z must come before W).
#'
#' @return LCM-SR outputs.
#' @export
#' @examples
#'
#' ## -- Example -- ##
#'
#' LCMSR(data.source="Data_A", 7, 2, X="EXPOSE", Y="INTENS")
#'

LCMSR <- function(data.source, no.waves, lag=1, p = 0.001, X, Y, Z="NULL", W = "NULL") {
  ...
  ## ----- Creating Model LCMSR ----- ###
  sink('LCMSR.txt') # Start writing script to LCMSR.txt

    cat("\n", "## ----- Specify the model (LCMSR) ----- ##", "\n")
    cat("\n", "LCMSR <- '")
    ...
    # -- Run Model LCMSRMLR -- #
    cat(rep("\n",2), "# -- Run Model LCMSRMLR -- #")
    cat(rep("\n",2), "  LCMSRMLR.fit <- suppressWarnings(lavaan::sem(LCMSR,")
    cat("\n", "   ", data.source, ",")
    cat("\n", "   missing = 'fiml',")
    cat("\n", "   meanstructure = TRUE,")
    cat("\n", "   information = 'observed',")
  ...
  sink()  # Stop writing to file LCMSR.txt
  ## ------------------------------- ##


  ## -- Execute LCMSR.txt and request summary outputs-- ##
  source('LCMSR.txt')
  print(lavaan::summary(LCMSRMLR.fit, fit.measure = TRUE, standardized = TRUE, rsq = TRUE))
  ## ------------------------------- ##
  ...
  parEst <- lavaan::parameterEstimates(LCMSRMLR.fit, remove.nonfree = TRUE)
  pest2 <- parEst[,5]  # Estimated Parameters
  pest3 <- lavaan::lavTech(LCMSRMLR.fit, what = "vcov", add.labels = TRUE)  # Estimated Variance-Covariance of Estimated Parameters
  mcmc <- MASS::mvrnorm(n=1000000, mu=pest2, Sigma=pest3, tol = 1e-6)  # Run 1,000,000 simulations
  names(pest2) <-colnames(pest3)  # Save Parameter Names to Estimated Parameters
  b.no <- nrow(mcmc)  # No. of successful simulated samples
...
## -- Prepare pest2 file with BCCI and PCI p-values -- ##
  ...
  ## ----- Start writing script to LCMSR.txt ----- ##
  sink('LCMSR.txt')
    cat("\n", "# Specify the model (LCMSR)", "\n")
    cat("\n", "LCMSR <- '")
   ...
   if (lag == 4) {
    ...
    # -- Run LCMSRMLR -- #
    cat(rep("\n",2), "  LCMSRMLR.fit <- lavaan::sem(LCMSR, ")
    cat("\n", "   ", data.source, ",")
    cat("\n", "   missing = 'fiml',")
    cat("\n", "   meanstructure = TRUE,")
    cat("\n", "   information = 'observed',")
    ...
    # Request summary outputs
    cat(rep("\n",2), "  print(lavaan::summary(LCMSRMLR.fit, fit.measure = TRUE, standardized = TRUE, rsq = TRUE))", "\n")
